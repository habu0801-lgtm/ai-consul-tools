...
店舗の**月次シフト作成を自動化**した仕組み。
試作（Streamlit）から始め、**現場で回るGASシステム**に作り直して、店舗で運用しています。

**自分が毎月やっていた業務を、自分で仕組みに変えたもの**です。
使う側の事情が最初から分かっている状態で作れたことが、そのまま設計に出ています。
...
作業時間が1/4〜1/6になっただけでなく、**非エンジニアの運営管理者がボタン操作だけで**シフト案を生成できる状態にしました。
...
運営管理者がスプレッドシートを書き換えるだけで運用が回るようになりました。
...
運営管理者が「今月どこから触ればいいか」で迷わなくなり、毎月の問い合わせが不要になりました。
